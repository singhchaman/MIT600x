...
class Patient(object):
    ...
    def update(self):
        ...
        for virus in self.viruses:
            if virus.doesClear():
                v.remove(virus)
        popDensity=len(v)/float(self.getMaxPop())
        # Using float for answer to be a float rather than an int
        self.viruses=v
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            if drug in self.resistances.keys():
                Trigger = Trigger and self.isResistantTo(drug)
            else:
                self.resistances[drug] = False
                Trigger = False            
        newResistances = {}
        if Trigger == True and random.random() <= self.maxBirthProb * (1 - popDensity):
